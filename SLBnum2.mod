...
//     r_s = r + gamma * stepup;
	...
	rr = r/pi(+1);
// New
	[name='Fisherian equation for SLB']
	rrs = r_s/pi;
	[name='Sustainium equation']
	r_s = r - p_s;
//     [name='Entrepreneur borrowing constraint']
...
steady_state_model;
	...
	// --- Initial guess and solve for chi_s to match SLB Euler ---
	mu     = 0.3;
	e      = sig * (1 - mu) * y^(1 - varphi);
	k      = alpha * (mc - theta1 * mu^theta2 - tau*(1 - varphi)*sig*(1 - mu)*y^(-varphi)) * y / ((rr - phi_E * mk) / (1 - phi_E) - (1 - delta));
	A      = y / (k^alpha * h^(1 - alpha));
	i      = delta * k;
	w      = (1 - alpha) * mc * y / h / (1 + mh * phi_E);
	l      = mk * k / rr - mh * w * h;
	...
	// --- Compute SLB bond holdings ---
	b_s    = mk * k / rr - mmu * y * theta1 * mu^theta2;
	e_s    = 1;
	chi_s  = (1 - beta_H * rr / pi) * lb_H * b_s^sigmaS / e_s;

	// --- Now compute SLB rate ---
	r_s    = r - p_s;
	rrs    = r_s / pi;
	phiS   = 1 - beta_E * rs;
