...
mh      = 0.2;		% Labor borrowing constraint 0.1; % 
...
model;
    ...
    [name='FOC mu']
    ((tau*sig*y^(1-varphi) + rho * xi * d_t)/(theta2*theta1))^(1/(theta2-1)) = mu;
    ...
    [name='SLB share policy rule']
    xi = rho_xi * xi(-1) + (1- rho_xi)*(theta1 * theta2 / gamma) * mu^(theta2 -1);
...
steady_state_model;
    ...
    y       = y0;
    tau     = tau0;
    mu		= ((tau*sig*y^(1-varphi) + rho * xi * d_t)/(theta2*theta1))^(1/(theta2-1));
	e       = sig * (1 - mu) * y^(1 - varphi);
    g       = gy * y;
    pi      = piss;
    rr      = 1 / beta_H;
