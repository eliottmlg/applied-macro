...
var e_a e_g e_c e_m e_i e_r e_t e_p e_mh;


varexo eta_a eta_g eta_c eta_m eta_i eta_r eta_t eta_p eta_mh;
...
steady_state_model;
	...
	chi		= w*lb_H/(h^sigmaL);
	g 		= gy*y;
	e_a 	= 1; e_g 	= 1; e_c 	= 1; e_m 	= 1; e_i 	= 1; e_r 	= 1; e_t 	= 1; e_p = 1; e_mh = 1;
...
%%% estimation of the model
    ...
    mh_nblocks=1, 
    nograph,
    mode_file='noE/Output/noE_mean.mat'); //,				% number of mcmc chains
...
stoch_simul(irf=30, order=1, graph) e;
shock_decomposition(parameter_set=posterior_mode) l_obs co2_obs phi_E mu;
